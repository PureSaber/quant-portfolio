...
def test_regime_scale_flows_into_portfolio(tmp_path) -> None:
    ...
    pd.DataFrame({"date": pd.date_range("2024-01-01", periods=80), "close": range(80)}).to_csv(series, index=False)
    inputs = [{"name": "eq", "asset_class": "equity", "path": str(series), "date_col": "date", "value_col": "close"}]
    rules = {"vol_window": 10, "vol_lookback": 30, "vol_percentile_threshold": 0.8, "return_window": 10}
    ...
    cfg.write_text(
        yaml.safe_dump(
            {
                "strategies": [
                    {"name": "eq", "nav": str(nav), "weight": 1.0, "position_scale": result.position_scale},
                ]
            }
        ),
        encoding="utf-8",
